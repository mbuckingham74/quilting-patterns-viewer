#!/usr/bin/env python3
"""
AI-based orientation detection for quilting pattern thumbnails.

Uses Claude Vision to analyze each thumbnail and determine if it needs rotation.
Stores results in a new table for review.

Usage:
    python detect_orientation.py [--limit N] [--batch-size N] [--dry-run]
...
# The prompt for Claude to analyze orientation
ORIENTATION_PROMPT = """Analyze this quilting pattern thumbnail image. These are black line art designs on white backgrounds used for quilting.

Determine if the pattern appears to be correctly oriented or if it needs rotation.

Consider:
- If there are recognizable objects (flowers, animals, text, etc.), are they right-side up?
- If the pattern has a clear "top" and "bottom", is it oriented correctly?
- Border patterns or repeating designs may not have a clear orientation - mark these as "correct"
- Abstract or symmetrical patterns with no clear orientation should be marked as "correct"

Respond with ONLY a JSON object (no markdown, no explanation):
{
  "orientation": "correct" | "rotate_90_cw" | "rotate_90_ccw" | "rotate_180",
  "confidence": "high" | "medium" | "low",
  "reason": "brief explanation"
}

If the pattern is abstract/symmetrical with no clear up/down, use:
{"orientation": "correct", "confidence": "high", "reason": "symmetrical/abstract pattern"}"""


async def fetch_patterns_to_analyze(session, limit=None):
    """Fetch patterns that haven't been analyzed yet."""
    # First check if the orientation_analysis table exists
    url = f"{SUPABASE_URL}/rest/v1/orientation_analysis?select=pattern_id&limit=1"
    async with session.get(url, headers=HEADERS) as response:
        if response.status == 404 or 'does not exist' in await response.text():
            ...
            url = f"{SUPABASE_URL}/rest/v1/orientation_analysis?select=pattern_id"
...
async def process_batch(session, patterns, dry_run=False):
    ...
    for pattern in patterns:
        ...
        if not image_data:
            results.append((pattern_id, None, "download_failed"))
            continue

        # Analyze with Claude
        analysis = await analyze_image(session, image_data)
        if not analysis:
            results.append((pattern_id, None, "analysis_failed"))
            continue
        ...
        results.append((pattern_id, analysis, "success"))
...
async def main():
    parser = argparse.ArgumentParser(description='AI orientation detection for pattern thumbnails')
    ...
    print("Fetching patterns to analyze...")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        print(f"  Correct orientation: {total_correct}")
        print(f"  Needs rotation: {total_needs_rotation}")
        print(f"  Failed: {total_failed}")

        if rotation_needed:
            print(f"\nPatterns needing rotation:")
            for p in rotation_needed[:20]:
                print(f"  {p['id']}: {p['action']} ({p['confidence']}) - {p['reason']}")
            if len(rotation_needed) > 20:
                print(f"  ... and {len(rotation_needed) - 20} more")
